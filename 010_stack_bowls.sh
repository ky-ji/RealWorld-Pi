#!/bin/bash
# ══════════════════════════════════════════════════════════════
#  Stack Bowls - Pi0.5 LoRA Fine-tuning Script
#  Task: Stack three bowls together
#  Model: Pi0.5 with LoRA (JAX, multi-GPU data parallel)
#  Action space: 7D [x, y, z, ax, ay, az, gripper] (axis-angle)
#
#  Usage:
#    bash stack_bowls.sh              # 默认使用 GPU 0
#    bash stack_bowls.sh 1            # 使用 GPU 1
#    bash stack_bowls.sh 0,1,2,3      # 使用 GPU 0,1,2,3 (四卡并行)
#    bash stack_bowls.sh 4,5,6,7      # 使用 GPU 4,5,6,7
# ══════════════════════════════════════════════════════════════
...
CHECKPOINT_DIR="./checkpoints/stack_bowls_lora_0208"

# Checkpoint 保存间隔（按步数缩放以保持相同 epoch 间隔）
SAVE_INTERVAL=$((5000 / NUM_GPUS > 500 ? 5000 / NUM_GPUS : 500))

# Checkpoint 保留策略: step % KEEP_PERIOD == 0 的 checkpoint 永久保留，
# 其余会被新的 checkpoint 覆盖（设为 null 则保留所有）。
...
#    如果修改此值，需同步修改 config.py 中 pi05_stack_bowls_lora 配置的
...
echo "  (forced recomputation — data has been updated with"
echo "   quaternion temporal consistency fix)"
echo "============================================================"

NORM_STATS_PATH="./assets/${CONFIG_NAME}/010_stack_bowls_0209/norm_stats.json"

# 强制删除旧的 norm_stats（数据更新后必须重新计算）
if [ -f "${NORM_STATS_PATH}" ]; then
    echo "  Removing old norm_stats: ${NORM_STATS_PATH}"
    rm -f "${NORM_STATS_PATH}"
fi
...
DATA_DIR = pathlib.Path('/data1/vla-data/processed/PI/data/010_stack_bowls_0209')
...
output_path = pathlib.Path('./assets/${CONFIG_NAME}/010_stack_bowls_0209')
